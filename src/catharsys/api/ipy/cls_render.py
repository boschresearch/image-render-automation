...
TIpyRender = ForwardRef("IpyRender")
...
def RenderActionResultData(_xData: CResultData, *, sRenderLib: str):
    ...
    epRenderer = plugin.SelectEntryPointFromDti(
        sGroup=f"catharsys.ipy.render.{sRenderLib}",
        sTrgDti=_xData.sDti,
        sTypeDesc=f"catharsys '{sRenderLib}' ipython render class",
    )
...
########################################################################################
class CIPyRender(CIPyRenderBase):
    @property
    def sRenderLib(self):
    ...
    def __init__(
        self,
        *,
        xIpyRender: Union[CIPyRenderBase, TIpyRender, None] = None,
        sRenderLib: str = "std",
    ):

        super().__init__(xIpyRender)

        if isinstance(xIpyRender, CIPyRender):
            self._sRenderLib = xIpyRender.sRenderLib
        else:
