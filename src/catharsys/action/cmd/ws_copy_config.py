...
def AddArgParseArguments(_parseArgs):
    ...
    _parseArgs.add_argument("source", nargs=1, default=[None])
    _parseArgs.add_argument("target", nargs=1, default=[None])
...
def RunCmd(_argsCmd, _lArgs):
    ...
    from . import ws_copy_config_impl as impl
    ...
    impl.Copy(
        sCfgNameSource=argsSubCmd.source[0],
        sCfgNameTarget=argsSubCmd.target[0],
